...
impl HypercoreBuilder {
    ...
    /// Open a core from just its 32-byte key, without a key pair. Required
    /// for v11 manifest cores (where the key is the manifest hash, not an
    /// ed25519 public key). The core is read-only; proofs can be verified
    /// only once the manifest is known (see `manifest` below or
    /// [`Hypercore::set_manifest`]).
    pub fn raw_key(mut self, key: [u8; 32]) -> Self {
        self.options.key = Some(key);
        self
    }

    /// Set the manifest for a core opened with `raw_key`. Must hash to the
    /// key. If not given, the manifest can be received from a peer and
    /// applied with [`Hypercore::set_manifest`].
    pub fn manifest(mut self, manifest: crate::crypto::Manifest) -> Self {
        self.options.manifest = Some(manifest);
        self
    }
