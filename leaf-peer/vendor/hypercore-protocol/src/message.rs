...
pub(crate) fn decode_unframed_channel_messages(
    ...
    if og_len >= 3 && buf[0] == 0x00 {
        // Control session (channel 0) message: type 0 = batch, 1 = open,
        // 2 = reject, 3 = close. Mirrors protomux's _oncontrolsession.
        if buf[1] == 0x00 {
            let (_, mut buf) = take_array::<2>(buf)?;
            // Batch of messages: [remoteId] then repeated [len][typed message],
            // where len == 0 signals "next uint is a new remoteId".
            ...
            while !buf.is_empty() {
                ...
                if channel_message_length == 0 {
                    // Channel switch marker
                    (current_channel, buf) = u64::decode(buf)?;
                    continue;
                }
...
\tInitial buffer size [{og_len}]",
                ...
                let (message_buf, rest) = buf.split_at(channel_message_length);
                buf = rest;
                if current_channel == 0 {
                    // Control message inside a batch (corked muxer), e.g. a
                    // channel open from corestore.
                    let (typ, control_buf) = u64::decode(message_buf)?;
                    match typ {
                        1 => {
                            let (channel_message, _) =
                                ChannelMessage::decode_open_message(control_buf)?;
                            messages.push(channel_message);
                        }
                        3 => {
                            let (channel_message, _) =
                                ChannelMessage::decode_close_message(control_buf)?;
                            messages.push(channel_message);
                        }
                        2 => {
                            // Session reject: remote refuses a channel we
                            // opened. Nothing to decode beyond the id; skip.
                            trace!("Remote rejected a session (batched), ignoring");
                        }
                        t => {
                            trace!("Ignoring unknown control message type {t} in batch");
                        }
                    }
                } else {
                    let (channel_message, _) =
                        ChannelMessage::decode_with_channel(message_buf, current_channel)?;
                    trace!(
                        "Decoded ChannelMessage::{:?} using [{} bytes]",
                        channel_message.message, channel_message_length
                    );
                    messages.push(channel_message);
                }
            }
            ...
            // Session reject; consume the rejected session id and move on.
            let (_session_id, rest) = u64::decode(&buf[2..])?;
            trace!("Remote rejected a session, ignoring");
            Ok((vec![], og_len - rest.len()))
        } else {
            Err(io::Error::new(
                io::ErrorKind::InvalidData,
...
/// Open/Close are control-session messages: inside a batch they travel on
/// channel 0 with an explicit type tag, exactly like protomux does.
fn batch_channel(message: &ChannelMessage) -> u64 {
...
fn batch_type_tag(message: &ChannelMessage) -> Option<u8> {
    match message.message {
        Message::Open(_) => Some(1),
        Message::Close(_) => Some(3),
        _ => None,
    }
}
...
fn vec_channel_messages_encoded_size(messages: &[ChannelMessage]) -> Result<usize, EncodingError> {
    Ok(match messages {
        ...
        msgs => {
            ...
            let mut current_channel: u64 = batch_channel(&msgs[0]);
            out += current_channel.encoded_size()?;
            for message in msgs.iter() {
                let channel = batch_channel(message);
                if channel != current_channel {
                    // Channel changed, need to add a 0x00 in between and then the new
                    // channel
                    out += CHANNEL_CHANGE_SEPERATOR.len() + channel.encoded_size()?;
                    current_channel = channel;
                }
                let type_len: usize = if batch_type_tag(message).is_some() { 1 } else { 0 };
                let message_length = message.message.encoded_size()? + type_len;
        ...
        Self: Sized,
    {
        ...
        match vec {
            ...
            msgs => {
                ...
                for msg in msgs {
                    let channel = batch_channel(msg);
                    if channel != current_channel {
                        rest = write_array(&CHANNEL_CHANGE_SEPERATOR, rest)?;
                        rest = channel.encode(rest)?;
                        current_channel = channel;
                    }
                    let type_tag = batch_type_tag(msg);
                    let type_len: usize = if type_tag.is_some() { 1 } else { 0 };
                    let msg_len = msg.message.encoded_size()? + type_len;
                    rest = (msg_len as u64).encode(rest)?;
                    if let Some(tag) = type_tag {
                        rest = write_array(&[tag], rest)?;
                    }
