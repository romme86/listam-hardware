...
use hypercore_schema::{
    DataBlock, DataHash, DataSeek, DataUpgrade, Manifest, Proof, RequestBlock, RequestSeek,
    RequestUpgrade,
};
...
pub struct Synchronize {
    ...
    /// Sender has the core's manifest (hypercore 11)
    pub has_manifest: bool,
    /// Sender allows pushed blocks (hypercore 11)
    pub allow_push: bool,
}
...
impl CompactEncoding for Synchronize {
    ...
    fn encode<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut [u8], EncodingError> {
        ...
        flags |= if self.has_manifest { 8 } else { 0 };
        flags |= if self.allow_push { 16 } else { 0 };
        ...
        Self: Sized,
    {
        ...
        let has_manifest = flags & 8 != 0;
        let allow_push = flags & 16 != 0;
        Ok((
            Synchronize {
                fork,
                length,
...
pub struct Data {
    ...
    /// Core manifest, sent in response to a [Request] with `manifest: true`
    /// (hypercore 11)
...
impl CompactEncoding for Data {
    fn encoded_size(&self) -> Result<usize, EncodingError> {
        ...
        opt_encoded_size!(&self.manifest, out);
        Ok(out)
    }
    ...
    fn encode<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut [u8], EncodingError> {
        ...
        flags |= if self.seek.is_some() { 4 } else { 0 };
        flags |= if self.upgrade.is_some() { 8 } else { 0 };
        flags |= if self.manifest.is_some() { 16 } else { 0 };
        let rest = write_array(&[flags], buffer)?;
        ...
        let rest = opt_encoded_bytes!(&self.manifest, rest);
        Ok(rest)
    }
        ...
        Self: Sized,
    {
        ...
        let (manifest, rest) = maybe_decode!(flags & 16 != 0, Manifest, rest);
        Ok((
            Data {
                request,
                fork,
...
impl Data {
    /// Transform Data message into a [`Proof`]. The manifest, if any, must be
    /// applied to the core separately (see `Hypercore::set_manifest`) before
    /// verifying the proof.
...
pub struct NoData {
    ...
    pub request: u64,
    /// Reason the request could not be fulfilled, 0 if not given (hypercore 11)
    pub reason: u64,
}

impl CompactEncoding for NoData {
    fn encoded_size(&self) -> Result<usize, EncodingError> {
        let mut out = sum_encoded_size!(self.request) + 1; // flags
        if self.reason != 0 {
            out += self.reason.encoded_size()?;
        }
        Ok(out)
    }

    fn encode<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut [u8], EncodingError> {
        let rest = self.request.encode(buffer)?;
        let rest = (if self.reason != 0 { 1u64 } else { 0u64 }).encode(rest)?;
        if self.reason != 0 {
            return self.reason.encode(rest);
        }
        Ok(rest)
    }
    ...
    {
        let (request, rest) = u64::decode(buffer)?;
        // flags + reason are recent wire additions; tolerate their absence
        let (flags, rest) = if rest.is_empty() {
            (0, rest)
        } else {
            u64::decode(rest)?
        };
        let (reason, rest) = if flags & 1 != 0 {
            u64::decode(rest)?
        } else {
            (0, rest)
        };
        Ok((Self { request, reason }, rest))
    }
}
...
pub struct Want {
    ...
    /// Length
    pub length: u64,
    /// Any block in the range satisfies the want (hypercore 11)
    pub any: bool,
}

impl CompactEncoding for Want {
    fn encoded_size(&self) -> Result<usize, EncodingError> {
        let mut out = sum_encoded_size!(self.start, self.length);
        if self.any {
            out += 1;
        }
        Ok(out)
    ...
    fn encode<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut [u8], EncodingError> {
        let rest = map_encode!(buffer, self.start, self.length);
        if self.any {
            return 1u64.encode(rest);
        }
        Ok(rest)
    }
        ...
        Self: Sized,
    {
        ...
        // trailing flags are a recent wire addition; tolerate their absence
        let (flags, rest) = if rest.is_empty() {
            (0, rest)
        } else {
            u64::decode(rest)?
...
pub struct Unwant {
    ...
    /// Length
    pub length: u64,
    /// Any block in the range satisfies the unwant (hypercore 11)
    pub any: bool,
}
