...
pub(crate) struct Header {
    ...
    /// Core key: the ed25519 public key for compat cores, the manifest hash
    /// for v11 manifest cores.
    pub(crate) key: [u8; 32],
    /// Manifest; `None` for a v11 core whose manifest was not yet received.
    pub(crate) manifest: Option<Manifest>,
    /// Key pair; `None` for cores opened from just a key (read-only mirrors).
...
impl Header {
    ...
    /// Creates a new Header for a core opened from just its key (and
    /// optionally its manifest), e.g. a v11 core mirrored from a peer.
    pub(crate) fn new_from_key(key: [u8; 32], manifest: Option<Manifest>) -> Self {
        Self {
            key,
            manifest,
            key_pair: None,
...
impl CompactEncoding for Header {
    fn encoded_size(&self) -> Result<usize, EncodingError> {
        let mut out = 1 + 1 + 32; // version + flags + key
        if let Some(manifest) = &self.manifest {
            out += manifest.encoded_size()?;
        }
        if let Some(key_pair) = &self.key_pair {
            out += key_pair.encoded_size()?;
        }
        Ok(out + sum_encoded_size!(self.user_data, self.tree, self.hints))
    }
    ...
    fn encode<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut [u8], EncodingError> {
        ...
        let mut rest = write_array(&[1, flags], buffer)?;
        rest = write_array(&self.key, rest)?;
        if let Some(manifest) = &self.manifest {
            rest = manifest.encode(rest)?;
        }
        if let Some(key_pair) = &self.key_pair {
            rest = key_pair.encode(rest)?;
        }
        Ok(map_encode!(rest, self.user_data, self.tree, self.hints))
    }
    ...
    where
        Self: Sized,
    {
        let ([_version, flags], rest) = take_array::<2>(buffer)?;
        let (key, mut rest) = take_array::<32>(rest)?;
        let manifest = if flags & 2 != 0 {
            let (manifest, new_rest) = Manifest::decode(rest)?;
            rest = new_rest;
            Some(manifest)
        } else {
            None
        };
        let key_pair = if flags & 4 != 0 {
            let (key_pair, new_rest) = PartialKeypair::decode(rest)?;
            rest = new_rest;
            Some(key_pair)
        } else {
            None
        };
        let ((user_data, tree, hints), rest) =
            map_decode!(rest, [Vec<String>, HeaderTree, HeaderHints]);
        Ok((
            Header {
                key,
                manifest,
...
mod tests {
    ...
    fn encode_header() -> Result<(), EncodingError> {
        ...
        assert!(rest.is_empty());
        assert_eq!(
            header.key_pair.as_ref().unwrap().public,
            dec_header.key_pair.as_ref().unwrap().public
        );
        assert_eq!(header.tree.fork, dec_header.tree.fork);
        assert_eq!(header.tree.length, dec_header.tree.length);
        assert_eq!(header.key, dec_header.key);
        let manifest = header.manifest.as_ref().unwrap();
        let dec_manifest = dec_header.manifest.as_ref().unwrap();
        assert_eq!(manifest.version, dec_manifest.version);
        assert_eq!(
            manifest.signers[0].public_key,
            dec_manifest.signers[0].public_key
        );
        Ok(())
    }

    #[test]
    fn encode_header_from_key_only() -> Result<(), EncodingError> {
        let header = Header::new_from_key([0xbe; 32], None);
        let encoded = to_encoded_bytes!(&header);
        let ((dec_header,), rest) = map_decode!(&encoded, [Header]);
        assert!(rest.is_empty());
        assert_eq!(dec_header.key, [0xbe; 32]);
        assert!(dec_header.manifest.is_none());
        assert!(dec_header.key_pair.is_none());
        Ok(())
    }
}
