    ...
    pub(crate) fn open(
        ...
        match infos {
            ...
            Some(infos) => {
                ...
                let signature: Option<Box<[u8]>> = if !header_tree.signature.is_empty() {
                    Some(header_tree.signature.clone())
                } else {
                    None
                };
...
fn verify_upgrade(
    ...
    changeset.fork = fork;
    changeset.verify_and_set_signature(&upgrade.signature, verifier)?;
    Ok(q.extra.is_none())
}
