...
impl Storage {
    ...
    pub(crate) async fn flush_infos(&mut self, infos: &[StoreInfo]) -> Result<(), HypercoreError> {
        ...
        // Sync every store this flush touched: FATFS (ESP-IDF) only persists
        // a file's directory entry — including its SIZE — on fsync/close, so
        // without this a hard reset reloads freshly written stores as
        // length=0 even though the data made it to the flash chain.
        let mut touched: Vec<Store> = Vec::with_capacity(4);
        for info in infos.iter() {
            if !touched.contains(&info.store) {
                touched.push(info.store.clone());
            }
        }
        for store in touched {
            self.get_random_access_mut(&store)
                .sync_all()
                .await
                .map_err(map_random_access_err)?;
        }
    ...
    /// New in-memory storage with a custom `random-access-memory` page size.
    ///
    /// The default page size is 1 MiB, allocated in full on the first write to
    /// each page — far too greedy on memory-constrained targets (an ESP32
    /// mirroring several small cores exhausts PSRAM and aborts). Pass a small
    /// page size (e.g. 64 KiB) there; hypercore blocks are written
    /// sequentially so smaller pages only trade a little bookkeeping for a
    /// much smaller, fragmentation-friendly footprint.
    #[instrument(err)]
    pub async fn new_memory_with_page_size(page_size: usize) -> Result<Self, HypercoreError> {
        let create = move |_| {
            async move {
                Ok(Box::new(RandomAccessMemory::new(page_size))
    ...
    /// New storage backed by blocking `std::fs` files under `dir`, using the
    /// [`RandomAccessFile`] backend. Works anywhere `std` is available
    /// (including ESP-IDF FATFS), no async runtime required. Persists across
    /// restarts, so a leaf re-opens its mirrored cores from flash instead of
    /// re-downloading them.
    #[instrument(err)]
    pub async fn new_file_storage(dir: &Path) -> Result<Self, HypercoreError> {
        ...
        let storage = move |store: Store| {
            ...
            async move {
                ...
                Ok(Box::new(RandomAccessFile::open(file::store_path(&dir, name))?)
                    as Box<dyn StorageTraits + Send>)
            }
            .boxed()
        };
        Self::open(storage, false).await
    }
