...
impl ChannelMap {
    ...
    /// Whether a LOCAL end is already attached for this discovery key.
    /// `attach_local` on an existing entry silently overwrites the previous
    /// local id, which desyncs the wire (two Opens for one key — JS protomux
    /// pairs only the first; the second channel goes dead). Callers use this
    /// to make `open` idempotent per connection.
    pub(crate) fn has_local_channel(&self, discovery_key: &[u8]) -> bool {
        let hdkey = hex::encode(discovery_key);
        self.channels
            .get(&hdkey)
            .map(|channel| channel.local_id().is_some())
            .unwrap_or(false)
    }
