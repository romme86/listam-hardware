...
/// Identity to create a fresh core from: either a full/partial key pair
/// (compat cores this instance may write to) or just a key with an optional
/// manifest (v11 cores mirrored from peers).
#[derive(Debug, Clone)]
pub(crate) enum CoreIdentity {
...
impl Oplog {
        ...
        match info {
            ...
            Some(info) => {
                ...
                let mut outcome: OplogOpenOutcome = if let Some(h1_outcome) = h1_outcome {
                    ...
                    // There is nothing in the oplog, start from fresh given identity.
                    Self::fresh(identity.clone())?
                } else {
                    // The storage is empty and no identity given, erroring
    ...
    pub(crate) fn update_header_with_changeset(
        ...
        let entry: Entry = if changeset.upgraded {
            ...
            let signature: Box<[u8]> = changeset
                .signature
                .clone()
    ...
    fn fresh(identity: CoreIdentity) -> Result<OplogOpenOutcome, HypercoreError> {
        let entries_length: u64 = 0;
        let entries_byte_length: u64 = 0;
        let header = match identity {
            CoreIdentity::KeyPair(key_pair) => Header::new(key_pair),
            CoreIdentity::Key { key, manifest } => Header::new_from_key(key, manifest),
        };
