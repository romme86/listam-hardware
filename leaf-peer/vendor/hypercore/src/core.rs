...
use crate::{
    ...
    crypto::{CoreVerifier, Manifest, PartialKeypair, generate_signing_key},
    data::BlockStore,
    oplog::{CoreIdentity, Header, MAX_OPLOG_ENTRIES_BYTE_SIZE, Oplog},
    storage::Storage,
...
pub struct Hypercore {
    pub(crate) key_pair: Option<PartialKeypair>,
    pub(crate) key: [u8; 32],
    pub(crate) verifier: CoreVerifier,
...
impl Hypercore {
    ...
    pub(crate) async fn new(
        ...
        let identity: Option<CoreIdentity> = if options.open {
            if options.key_pair.is_some() || options.key.is_some() {
                return Err(HypercoreError::BadArgument {
                    context: "Key pair or key can not be used when building an openable hypercore"
                        .to_string(),
                });
            }
            None
        } else if let Some(key) = options.key.take() {
            if options.key_pair.is_some() {
                return Err(HypercoreError::BadArgument {
                    context: "Only one of key pair and key can be given".to_string(),
                });
            }
            if let Some(manifest) = &options.manifest {
                // Validates that the manifest matches the key
                CoreVerifier::from_key_and_manifest(key, Some(manifest))?;
            }
            Some(CoreIdentity::Key {
                key,
                manifest: options.manifest.take(),
            })
        } else {
            Some(CoreIdentity::KeyPair(options.key_pair.take().unwrap_or_else(
                || {
        ...
        // Open/create oplog
        let mut oplog_open_outcome = match Oplog::open(&identity, None)? {
            Either::Right(value) => value,
            Either::Left(instruction) => {
                let info = storage.read_info(instruction).await?;
                match Oplog::open(&identity, Some(info))? {
                    Either::Right(value) => value,
                    Either::Left(_) => {
        ...
        if let Some(entries) = oplog_open_outcome.entries {
            for entry in entries.iter() {
                ...
                if let Some(tree_upgrade) = &entry.tree_upgrade {
                    ...
                    changeset.signature = Some(tree_upgrade.signature.clone());
        ...
        let key = header.key;
        let verifier = CoreVerifier::from_key_and_manifest(key, header.manifest.as_ref())?;
    ...
    pub fn info(&self) -> Info {
        Info {
            ...
            writeable: self
                .key_pair
                .as_ref()
                .is_some_and(|key_pair| key_pair.secret.is_some()),
        }
    }
    ...
    pub async fn append_batch<A: AsRef<[u8]>, B: AsRef<[A]>>(
        ...
        if !matches!(self.verifier, CoreVerifier::Compat(_)) {
            // Signing version >= 1 manifest cores is not implemented
            return Err(HypercoreError::NotWritable);
        }
        let secret_key = match self.key_pair.as_ref().and_then(|kp| kp.secret.as_ref()) {
            Some(key) => key,
    ...
    /// Access the key pair, if the core has one. Cores opened from just a
    /// key (e.g. v11 manifest cores mirrored from peers) have none.
    pub fn key_pair(&self) -> Option<&PartialKeypair> {
        self.key_pair.as_ref()
    }

    /// The core's key: the ed25519 public key for compat cores, the manifest
    /// hash for v11 manifest cores.
    pub fn key(&self) -> [u8; 32] {
        self.key
    }

    /// The core's manifest, if known.
    pub fn manifest(&self) -> Option<&Manifest> {
        self.header.manifest.as_ref()
    }

    /// Set the manifest of a core that was opened with just its key.
    /// Validates that the manifest hashes to the core key, persists it, and
    /// enables proof verification. Idempotent if a manifest is already set.
    #[instrument(err, skip_all)]
    pub async fn set_manifest(&mut self, manifest: Manifest) -> Result<(), HypercoreError> {
        if self.header.manifest.is_some() {
            return Ok(());
        }
        self.verifier = CoreVerifier::from_key_and_manifest(self.key, Some(&manifest))?;
        self.header.manifest = Some(manifest);
        self.flush_bitfield_and_tree_and_oplog(false).await?;
        Ok(())
    }
    ...
    pub async fn make_read_only(&mut self) -> Result<bool, HypercoreError> {
            ...
            .is_some_and(|key_pair| key_pair.secret.is_some())
        {
            ...
            if let Some(key_pair) = self.header.key_pair.as_mut() {
                key_pair.secret = None;
            }
    ...
    async fn verify_proof(&mut self, proof: &Proof) -> Result<MerkleTreeChangeset, HypercoreError> {
        match self.tree.verify_proof(proof, &self.verifier, None)? {
            Either::Right(value) => Ok(value),
            Either::Left(instructions) => {
                let infos = self.storage.read_infos_to_vec(&instructions).await?;
                match self
                    .tree
                    .verify_proof(proof, &self.verifier, Some(&infos))?
                {
                    Either::Right(value) => Ok(value),
...
pub(crate) mod tests {
    ...
    async fn core_create_proof_block_and_seek_1_no_upgrade() -> Result<(), HypercoreError> {
        ...
        let proof = hypercore
            .create_proof(
                ...
                None,
                Some(RequestSeek { bytes: 8, padding: 0 }),
                None,
            )
            .await?
            .unwrap();
    ...
    async fn core_create_proof_block_and_seek_2_no_upgrade() -> Result<(), HypercoreError> {
        ...
        let proof = hypercore
            .create_proof(
                Some(RequestBlock { index: 4, nodes: 2 }),
                None,
                Some(RequestSeek { bytes: 10, padding: 0 }),
                None,
            )
            .await?
            .unwrap();
    ...
    async fn core_create_proof_block_and_seek_3_no_upgrade() -> Result<(), HypercoreError> {
        ...
        let proof = hypercore
            .create_proof(
                Some(RequestBlock { index: 4, nodes: 2 }),
                None,
                Some(RequestSeek { bytes: 13, padding: 0 }),
                None,
            )
            .await?
            .unwrap();
    ...
    async fn core_create_proof_block_and_seek_to_tree_no_upgrade() -> Result<(), HypercoreError> {
        ...
        let proof = hypercore
            .create_proof(
                ...
                None,
                Some(RequestSeek { bytes: 26, padding: 0 }),
                None,
            )
            .await?
            .unwrap();
    ...
    async fn core_verify_and_apply_proof() -> Result<(), HypercoreError> {
        ...
        let mut clone = create_hypercore_with_data_and_key_pair(
            0,
            PartialKeypair {
                public: main.key_pair.as_ref().unwrap().public,
                secret: None,
            },
        )
        .await?;
