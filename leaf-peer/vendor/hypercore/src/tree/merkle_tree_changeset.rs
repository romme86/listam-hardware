...
use crate::{
    HypercoreError,
    crypto::{CoreVerifier, signable_tree, verify, verify_manifest_signature},
    sign,
};
...
pub(crate) struct MerkleTreeChangeset {
    ...
    /// Tree signature: raw 64-byte ed25519 for compat cores, the assembled
    /// multisig encoding for v11 manifest cores.
    pub(crate) signature: Option<Box<[u8]>>,
...
impl MerkleTreeChangeset {
    ...
    /// Verify and set signature using the core's verifier
    pub(crate) fn verify_and_set_signature(
        ...
        let hash = self.hash();
        match verifier {
            CoreVerifier::Compat(public_key) => {
                // v10: raw ed25519 signature over the v10 signable
                let parsed = Signature::try_from(signature).map_err(|_| {
                    HypercoreError::InvalidSignature {
                        context: "Could not parse signature".to_string(),
                    }
                })?;
                verify(public_key, &self.signable(&hash), Some(&parsed))?;
            }
            CoreVerifier::Manifest {
                hash: manifest_hash,
                manifest,
            } => {
                let tree_hash: [u8; 32] =
                    hash.as_ref()
                        .try_into()
                        .map_err(|_| HypercoreError::InvalidChecksum {
                            context: "Tree hash should be 32 bytes".to_string(),
                        })?;
                if !verify_manifest_signature(
                    ...
                    return Err(HypercoreError::InvalidSignature {
                        context: "Manifest signature verification failed".to_string(),
                    });
                }
            }
            CoreVerifier::Pending => {
                return Err(HypercoreError::InvalidSignature {
                    context: "Can not verify upgrade: core manifest not yet known".to_string(),
                });
            }
        }
